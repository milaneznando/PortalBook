...
public class BookController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Book>>> GetAllBooks(int pageNumber, int pageSize = 1)
    {
        try
        {
            var setPageSize = Math.Min(pageSize, 50);

            return Ok(await _bookService.ListAll(pageNumber, setPageSize));
        }
        catch (Exception ex)
        {
