...
* Utiliza Entity Framework Core para acesso a dados, migrações de banco de dados, e criação de *bundles*.
...
## Notas Importantes
* Testes: Testes unitários e de integração são altamente recomendados e devem ser adicionados ao projeto quando o tempo permitir.
* Implantação (Deployment): Considere utilizar conteinerização (Docker) para implantações simplificadas.

## Considerações para Desenvolvimento Futuro
* Testes: Implemente testes unitários e de integração para garantir a qualidade e robustez do código.
* Autenticação/Autorização: Implemente mecanismos para gerenciamento de usuários e controle de acesso.
* Frontend: Crie um frontend usando um framework adequado, como React, Angular ou Vue.js, para interagir com sua API.

Tempo total utilizado no teste: 6h
* Não consegui utilizar as 8h totais, por conta de tarefas diárias que não possibilitaram com que eu as utilizasse.